package com.qopuir.restapi.core.service;
...
import com.qopuir.restapi.core.dao.PodcastDao;
...
@Component
@Api(value = "podcasts", description = "Operations about podcasts")
@Path("/podcasts")
public class PodcastRestService {
	...
	@Produces({ MediaType.TEXT_HTML })
	@Transactional
	public Response createPodcastFromForm(@FormParam("title") String title, @FormParam("linkOnPodcastpedia") String linkOnPodcastpedia, @FormParam("feed") String feed, @FormParam("description") String description) {
	...
	@GET
	@Path("{id}")
	@ApiOperation(value = "Find podcast by ID", response = Podcast.class)
	@ApiResponses(value = {
		@ApiResponse(code = 404, message = "Podcast not found")
	})
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
